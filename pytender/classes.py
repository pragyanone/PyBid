...
class letter:
    def __init__(self, path, firm_name, firm_addr, jv, short_name, contract_name, IoB, contract_ID, to, days, line_of_cr, person, date):
        ...
        self.designation = 'Managing Director' if 'pvt' in self.firm_name.lower() else 'Proprietor'
    ...
    def save_file(self):
        filename = self.path + os.path.sep + self.heading + ' -' + self.short_name + '.docx'
...
class poa_self(declaration):
    ...
    def write_body(self):
        ...
        temp.add_run(self.firm_name + ', ' + self.firm_addr).bold = True
        temp.add_run('" hereby inform you that as I, myself do all the activities of the firm, I appoint myself as the authorized representative to do all the official and site activities of the project mentioned above.')
    ...
    def save_file(self):
        filename = self.path + os.path.sep + 'PoA Self' + ' -' + self.short_name + '.docx'
        self.doc.save(filename)